...
{
    public sealed class Request(SucursalOrder order, SucursalSearch search, PageParameters pageParameters) : SearchParameters<SucursalOrder, SucursalSearch>(order, search, pageParameters), IRequest<Result<IEnumerable<SucursalTableModel>>> { }

    internal sealed class Handler(IApplicationServices services) : IRequestHandler<Request, Result<IEnumerable<SucursalTableModel>>>
    {
        public async Task<Result<IEnumerable<SucursalTableModel>>> Handle(Request request, CancellationToken cancellationToken)
        {
            var tableModelList = await services
                .Sucursales.GetSucursales(request);
